...
function Mutton() {
  ...
  useEffect(() => {
    async function fetchMuttonData() {
      try {
        const response = await fetch("http://localhost:8000/api/mutton");
        const data = await response.json();
        return data;
      } catch (error) {
        throw new Error("Error fetching mutton data:", error);
      }
    }

    async function fetchData() {
      try {
        const data = await fetchMuttonData();
