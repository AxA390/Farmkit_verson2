...
function Cabbage() {
  ...
  useEffect(() => {
    async function fetchCabbageData() {
      try {
        const response = await fetch("http://localhost:8000/api/cabbage");
    ...
    async function fetchData() {
      try {
        const data = await fetchCabbageData();
...
// Your existing frontend code with the addToCart function
...
const addToCart = async (product) => {
  try {
    ...
    // Optionally, you can update the cart state or perform other actions after successfully adding the item to the cart
  } catch (error) {
    ...
    <div className="flex justify-around mt-5 mx-11">
      <div className="w-1/4 ml-11">
        ...
        <div className="product-name">
          <p className="text-3xl text-center font-bold">Cabbage</p>
        </div>
      </div>
