...
function Carrot() {
  ...
  useEffect(() => {
    async function fetchCarrotData() {
      try {
        const response = await fetch("http://localhost:8000/api/carrot");
    ...
    async function fetchData() {
      try {
        const data = await fetchCarrotData();
    ...
    <div className="flex justify-around mt-5 mx-11">
      <div className="w-1/4 ml-11">
        ...
        <div className="product-name">
          <p className="text-3xl text-center font-bold">Carrot</p>
        </div>
      </div>
