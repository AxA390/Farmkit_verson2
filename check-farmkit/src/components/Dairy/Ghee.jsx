...
function Ghee() {
  ...
  useEffect(() => {
    async function fetchGheeData() {
      try {
        const response = await fetch("http://localhost:8000/api/ghee");
        const data = await response.json();
        return data;
      } catch (error) {
        throw new Error("Error fetching ghee data:", error);
      }
    }

    async function fetchData() {
      try {
        const data = await fetchGheeData();
      ...
      <div className="flex justify-around mt-5 mx-11">
        ...
        <div className="infodetails ">
          ...
          {filteredFarmerDetails.length === 0 ? (
            ...
            filteredFarmerDetails.map((farmer, index) => (
              <div
