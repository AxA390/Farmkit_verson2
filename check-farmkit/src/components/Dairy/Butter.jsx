...
function Butter() {
  ...
  useEffect(() => {
    async function fetchButterData() {
      try {
        const response = await fetch("http://localhost:8000/api/butter");
        const data = await response.json();
        return data;
      } catch (error) {
        throw new Error("Error fetching butter data:", error);
      }
    }

    async function fetchData() {
      try {
        const data = await fetchButterData();
