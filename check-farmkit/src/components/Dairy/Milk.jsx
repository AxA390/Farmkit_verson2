...
function Milk() {
  ...
  useEffect(() => {
    async function fetchMilkData() {
      try {
        const response = await fetch("http://localhost:8000/api/milk");
        const data = await response.json();
        return data;
      } catch (error) {
        throw new Error("Error fetching milk data:", error);
      }
    }

    async function fetchData() {
      try {
        const data = await fetchMilkData();
      ...
      <div className="flex justify-around mt-5 mx-11">
        <div className="w-1/4 ml-11">
          ...
          <div className="product-name">
            <p className="text-3xl text-center font-bold">Milk</p>
          </div>
        </div>
